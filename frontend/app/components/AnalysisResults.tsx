import { AnalysisResultsProps } from '@/app/types/security';
      ...
      <div className="flex-1 overflow-auto">
        ...
        {!analysisResults && !error && (
          <div className="bg-gray-50 rounded-lg border border-border p-4 text-sm text-accent text-center">
            {isAnalyzing ? 'Analyzing flow...' : 'Upload and analyze Node-RED flow to see security assessment results here.'}
          </div>
        )}
                      ...
                      {analysisResults.issues.map((issue, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4 text-sm font-mono bg-gray-50 text-red-600 max-w-xs overflow-hidden">
                            <pre className="whitespace-pre-wrap wrap-break-word">{issue.node_id}</pre>
                          </td>
                          <td className="px-4 py-4 text-sm font-mono bg-green-50 text-green-700 max-w-xs overflow-hidden">
                            <pre className="whitespace-pre-wrap wrap-break-word">{issue.fix}</pre>
                          </td>
                        </tr>
                      ))}
                    </tbody>
